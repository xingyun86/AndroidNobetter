...
protected:
	...
	unsigned char     m_initVector[RD_MAX_IV_SIZE];
	unsigned int    m_uRounds;
	unsigned char     m_expandedKey[RD_MAX_ROUNDS+1][4][4];
...
public:
	...
	int Init(Mode mode, Direction dir, const unsigned char *key, KeyLength keyLen, const unsigned char *initVector);
...
protected:
	void KeySched(unsigned char key[RD_MAX_KEY_COLUMNS][4]);
	void KeyEncToDec();
