...
static const unsigned char g_S[256]=
{
...
static const unsigned char g_T1[256][4]=
{
...
static const unsigned char g_T2[256][4]=
{
...
static const unsigned char g_T3[256][4]=
{
...
static const unsigned char g_T4[256][4]=
{
...
static const unsigned char g_T5[256][4]=
{
...
static const unsigned char g_T6[256][4]=
{
...
static const unsigned char g_T7[256][4]=
{
...
static const unsigned char g_T8[256][4]=
{
...
static const unsigned char g_S5[256]=
{
...
static const unsigned char g_U1[256][4]=
{
...
static const unsigned char g_U2[256][4]=
{
...
static const unsigned char g_U3[256][4]=
{
...
static const unsigned char g_U4[256][4]=
{
...
static const unsigned int g_RCON[30]=
{
...
int CRijndael::Init(Mode mode, Direction dir, const unsigned char *key, KeyLength keyLen, const unsigned char *initVector)
{
	...
	unsigned char keyMatrix[RD_MAX_KEY_COLUMNS][4];
...
int CRijndael::BlockEncrypt(const unsigned char *input, int inputLen, unsigned char *outBuffer)
{
	int i, k, numBlocks;
	...
	switch(m_mode)
	{
		...
		case CBC:
			((unsigned int *)block)[0] = ((unsigned int *)m_initVector)[0] ^ ((unsigned int *)input)[0];
			((unsigned int *)block)[1] = ((unsigned int *)m_initVector)[1] ^ ((unsigned int *)input)[1];
			((unsigned int *)block)[2] = ((unsigned int *)m_initVector)[2] ^ ((unsigned int *)input)[2];
			((unsigned int *)block)[3] = ((unsigned int *)m_initVector)[3] ^ ((unsigned int *)input)[3];
			Encrypt(block,outBuffer);
			input += 16;
			for(i = numBlocks - 1;i > 0;i--)
			{
				((unsigned int *)block)[0] = ((unsigned int *)outBuffer)[0] ^ ((unsigned int *)input)[0];
				((unsigned int *)block)[1] = ((unsigned int *)outBuffer)[1] ^ ((unsigned int *)input)[1];
				((unsigned int *)block)[2] = ((unsigned int *)outBuffer)[2] ^ ((unsigned int *)input)[2];
				((unsigned int *)block)[3] = ((unsigned int *)outBuffer)[3] ^ ((unsigned int *)input)[3];
				outBuffer += 16;
				Encrypt(block,outBuffer);
				input += 16;
			}
...
#else  /* !STRICT_ALIGN */
			*((unsigned int *)iv[0]) = *((unsigned int *)(m_initVector   ));
			*((unsigned int *)iv[1]) = *((unsigned int *)(m_initVector + 4));
			*((unsigned int *)iv[2]) = *((unsigned int *)(m_initVector + 8));
			*((unsigned int *)iv[3]) = *((unsigned int *)(m_initVector +12));
#endif /* ?STRICT_ALIGN */
			for(i = numBlocks; i > 0; i--)
			{
				for(k = 0; k < 128; k++)
				{
					*((unsigned int *) block    ) = *((unsigned int *)iv[0]);
					*((unsigned int *)(block+ 4)) = *((unsigned int *)iv[1]);
					*((unsigned int *)(block+ 8)) = *((unsigned int *)iv[2]);
					*((unsigned int *)(block+12)) = *((unsigned int *)iv[3]);
					Encrypt(block,block);
					...
					iv[3][3] = (unsigned char)((iv[3][3] << 1) | (outBuffer[k/8] >> (7-(k&7))) & 1);
				}
			}
			break;
...
int CRijndael::PadEncrypt(const unsigned char *input, int inputOctets, unsigned char *outBuffer)
{
...
//			assert(padLen > 0 && padLen <= 16);
		...
		case CBC:
			...
			for(i = numBlocks; i > 0; i--)
			{
				((unsigned int *)block)[0] = ((unsigned int *)input)[0] ^ ((unsigned int *)iv)[0];
				((unsigned int *)block)[1] = ((unsigned int *)input)[1] ^ ((unsigned int *)iv)[1];
				((unsigned int *)block)[2] = ((unsigned int *)input)[2] ^ ((unsigned int *)iv)[2];
				((unsigned int *)block)[3] = ((unsigned int *)input)[3] ^ ((unsigned int *)iv)[3];
				Encrypt(block, outBuffer);
				iv = outBuffer;
				input += 16;
				outBuffer += 16;
...
int CRijndael::BlockDecrypt(const unsigned char *input, int inputLen, unsigned char *outBuffer)
{
	int i, k, numBlocks;
	unsigned char block[16], iv[4][4];
	...
	switch(m_mode)
	{
		...
		case CBC:
			...
			*((unsigned int *)iv[3]) = *((unsigned int *)(m_initVector+12));
			...
			{
				Decrypt(input, block);
				((unsigned int *)block)[0] ^= *((unsigned int *)iv[0]);
				((unsigned int *)block)[1] ^= *((unsigned int *)iv[1]);
				((unsigned int *)block)[2] ^= *((unsigned int *)iv[2]);
				((unsigned int *)block)[3] ^= *((unsigned int *)iv[3]);
#if STRICT_ALIGN
				memcpy(iv, input, 16);
				memcpy(outBuf, block, 16);
#else
				*((unsigned int *)iv[0]) = ((unsigned int *)input)[0]; ((unsigned int *)outBuffer)[0] = ((unsigned int *)block)[0];
				*((unsigned int *)iv[1]) = ((unsigned int *)input)[1]; ((unsigned int *)outBuffer)[1] = ((unsigned int *)block)[1];
				*((unsigned int *)iv[2]) = ((unsigned int *)input)[2]; ((unsigned int *)outBuffer)[2] = ((unsigned int *)block)[2];
				*((unsigned int *)iv[3]) = ((unsigned int *)input)[3]; ((unsigned int *)outBuffer)[3] = ((unsigned int *)block)[3];
#endif
				input += 16;
				outBuffer += 16;
			}
			...
			for(i = numBlocks; i > 0; i--)
			{
				for(k = 0; k < 128; k++)
				{
					...
					*((unsigned int *)(block+12)) = *((unsigned int *)iv[3]);
					...
					iv[3][1] = (iv[3][1] << 1) | (iv[3][2] >> 7);
					iv[3][2] = (iv[3][2] << 1) | (iv[3][3] >> 7);
					iv[3][3] = (unsigned char)((iv[3][3] << 1) | (input[k/8] >> (7-(k&7))) & 1);
					outBuffer[k/8] ^= (block[0] & 0x80) >> (k & 7);
				}
			}
			break;
...
int CRijndael::PadDecrypt(const unsigned char *input, int inputOctets, unsigned char *outBuffer)
{
	int i, numBlocks, padLen;
	...
	switch(m_mode){
		...
		case CBC:
			...
			{
				Decrypt(input, block);
				((unsigned int *)block)[0] ^= iv[0];
				((unsigned int *)block)[1] ^= iv[1];
				((unsigned int *)block)[2] ^= iv[2];
				((unsigned int *)block)[3] ^= iv[3];
				memcpy(iv, input, 16);
...
void CRijndael::KeySched(unsigned char key[RD_MAX_KEY_COLUMNS][4])
{
	int j, rconpointer = 0;
	...
	unsigned char tempKey[RD_MAX_KEY_COLUMNS][4];

	// Copy the input key to the temporary key matrix
	for(j = 0;j < uKeyColumns;j++)
		*((unsigned int *)(tempKey[j])) = *((unsigned int *)(key[j]));
	...
	for(j = 0;(j < uKeyColumns) && (r <= (int)m_uRounds); )
	{
		for(;(j < uKeyColumns) && (t < 4); j++, t++)
			*((unsigned int *)m_expandedKey[r][t]) = *((unsigned int *)tempKey[j]);
	...
	while(r <= (int)m_uRounds)
	{
		...
		if (uKeyColumns != 8)
		{
			...
			{
				*((unsigned int *)tempKey[j]) ^= *((unsigned int *)tempKey[j-1]);
			}
		}
		else
		{
...
void CRijndael::KeyEncToDec()
{
	...
	{
		w = m_expandedKey[r][0];
		*((unsigned int *)w) = *((unsigned int *)g_U1[w[0]]) ^ *((unsigned int *)g_U2[w[1]]) ^ *((unsigned int *)g_U3[w[2]]) ^ *((unsigned int *)g_U4[w[3]]);
		w = m_expandedKey[r][1];
		*((unsigned int *)w) = *((unsigned int *)g_U1[w[0]]) ^ *((unsigned int *)g_U2[w[1]]) ^ *((unsigned int *)g_U3[w[2]]) ^ *((unsigned int *)g_U4[w[3]]);
		w = m_expandedKey[r][2];
		*((unsigned int *)w) = *((unsigned int *)g_U1[w[0]]) ^ *((unsigned int *)g_U2[w[1]]) ^ *((unsigned int *)g_U3[w[2]]) ^ *((unsigned int *)g_U4[w[3]]);
...
void CRijndael::Encrypt(const unsigned char a[16], unsigned char b[16])
{
	int r;
	unsigned char temp[4][4];

    *((unsigned int *)temp[0]) = *((unsigned int *)(a   )) ^ *((unsigned int *)m_expandedKey[0][0]);
    *((unsigned int *)temp[1]) = *((unsigned int *)(a+ 4)) ^ *((unsigned int *)m_expandedKey[0][1]);
    *((unsigned int *)temp[2]) = *((unsigned int *)(a+ 8)) ^ *((unsigned int *)m_expandedKey[0][2]);
    *((unsigned int *)temp[3]) = *((unsigned int *)(a+12)) ^ *((unsigned int *)m_expandedKey[0][3]);
    *((unsigned int *)(b    )) = *((unsigned int *)g_T1[temp[0][0]])
						^ *((unsigned int *)g_T2[temp[1][1]])
						^ *((unsigned int *)g_T3[temp[2][2]])
						^ *((unsigned int *)g_T4[temp[3][3]]);
    *((unsigned int *)(b + 4)) = *((unsigned int *)g_T1[temp[1][0]])
						^ *((unsigned int *)g_T2[temp[2][1]])
						^ *((unsigned int *)g_T3[temp[3][2]])
						^ *((unsigned int *)g_T4[temp[0][3]]);
    *((unsigned int *)(b + 8)) = *((unsigned int *)g_T1[temp[2][0]])
						^ *((unsigned int *)g_T2[temp[3][1]])
						^ *((unsigned int *)g_T3[temp[0][2]])
						^ *((unsigned int *)g_T4[temp[1][3]]);
    *((unsigned int *)(b +12)) = *((unsigned int *)g_T1[temp[3][0]])
						^ *((unsigned int *)g_T2[temp[0][1]])
						^ *((unsigned int *)g_T3[temp[1][2]])
						^ *((unsigned int *)g_T4[temp[2][3]]);
	for(r = 1; r < (int)m_uRounds-1; r++)
	{
		*((unsigned int *)temp[0]) = *((unsigned int *)(b   )) ^ *((unsigned int *)m_expandedKey[r][0]);
		*((unsigned int *)temp[1]) = *((unsigned int *)(b+ 4)) ^ *((unsigned int *)m_expandedKey[r][1]);
		*((unsigned int *)temp[2]) = *((unsigned int *)(b+ 8)) ^ *((unsigned int *)m_expandedKey[r][2]);
		*((unsigned int *)temp[3]) = *((unsigned int *)(b+12)) ^ *((unsigned int *)m_expandedKey[r][3]);
	...
	*((unsigned int *)temp[0]) = *((unsigned int *)(b   )) ^ *((unsigned int *)m_expandedKey[m_uRounds-1][0]);
	*((unsigned int *)temp[1]) = *((unsigned int *)(b+ 4)) ^ *((unsigned int *)m_expandedKey[m_uRounds-1][1]);
	*((unsigned int *)temp[2]) = *((unsigned int *)(b+ 8)) ^ *((unsigned int *)m_expandedKey[m_uRounds-1][2]);
	*((unsigned int *)temp[3]) = *((unsigned int *)(b+12)) ^ *((unsigned int *)m_expandedKey[m_uRounds-1][3]);
	b[ 0] = g_T1[temp[0][0]][1];
	b[ 1] = g_T1[temp[1][1]][1];
	b[ 2] = g_T1[temp[2][2]][1];
	b[ 3] = g_T1[temp[3][3]][1];
	...
	*((unsigned int *)(b   )) ^= *((unsigned int *)m_expandedKey[m_uRounds][0]);
	*((unsigned int *)(b+ 4)) ^= *((unsigned int *)m_expandedKey[m_uRounds][1]);
	*((unsigned int *)(b+ 8)) ^= *((unsigned int *)m_expandedKey[m_uRounds][2]);
	*((unsigned int *)(b+12)) ^= *((unsigned int *)m_expandedKey[m_uRounds][3]);
}

void CRijndael::Decrypt(const unsigned char a[16], unsigned char b[16])
{
	int r;
	unsigned char temp[4][4];

    *((unsigned int *)temp[0]) = *((unsigned int *)(a   )) ^ *((unsigned int *)m_expandedKey[m_uRounds][0]);
    *((unsigned int *)temp[1]) = *((unsigned int *)(a+ 4)) ^ *((unsigned int *)m_expandedKey[m_uRounds][1]);
    *((unsigned int *)temp[2]) = *((unsigned int *)(a+ 8)) ^ *((unsigned int *)m_expandedKey[m_uRounds][2]);
    *((unsigned int *)temp[3]) = *((unsigned int *)(a+12)) ^ *((unsigned int *)m_expandedKey[m_uRounds][3]);

    *((unsigned int *)(b   )) = *((unsigned int *)g_T5[temp[0][0]])
           ^ *((unsigned int *)g_T6[temp[3][1]])
           ^ *((unsigned int *)g_T7[temp[2][2]])
           ^ *((unsigned int *)g_T8[temp[1][3]]);
	*((unsigned int *)(b+ 4)) = *((unsigned int *)g_T5[temp[1][0]])
           ^ *((unsigned int *)g_T6[temp[0][1]])
           ^ *((unsigned int *)g_T7[temp[3][2]])
           ^ *((unsigned int *)g_T8[temp[2][3]]);
	*((unsigned int *)(b+ 8)) = *((unsigned int *)g_T5[temp[2][0]])
           ^ *((unsigned int *)g_T6[temp[1][1]])
           ^ *((unsigned int *)g_T7[temp[0][2]])
           ^ *((unsigned int *)g_T8[temp[3][3]]);
	*((unsigned int *)(b+12)) = *((unsigned int *)g_T5[temp[3][0]])
           ^ *((unsigned int *)g_T6[temp[2][1]])
           ^ *((unsigned int *)g_T7[temp[1][2]])
           ^ *((unsigned int *)g_T8[temp[0][3]]);
	...
	*((unsigned int *)temp[0]) = *((unsigned int *)(b   )) ^ *((unsigned int *)m_expandedKey[1][0]);
	*((unsigned int *)temp[1]) = *((unsigned int *)(b+ 4)) ^ *((unsigned int *)m_expandedKey[1][1]);
	*((unsigned int *)temp[2]) = *((unsigned int *)(b+ 8)) ^ *((unsigned int *)m_expandedKey[1][2]);
	*((unsigned int *)temp[3]) = *((unsigned int *)(b+12)) ^ *((unsigned int *)m_expandedKey[1][3]);
	b[ 0] = g_S5[temp[0][0]];
	b[ 1] = g_S5[temp[3][1]];
	b[ 2] = g_S5[temp[2][2]];
	b[ 3] = g_S5[temp[1][3]];
	...
	b[14] = g_S5[temp[1][2]];
	b[15] = g_S5[temp[0][3]];
	*((unsigned int *)(b   )) ^= *((unsigned int *)m_expandedKey[0][0]);
	*((unsigned int *)(b+ 4)) ^= *((unsigned int *)m_expandedKey[0][1]);
	*((unsigned int *)(b+ 8)) ^= *((unsigned int *)m_expandedKey[0][2]);
	*((unsigned int *)(b+12)) ^= *((unsigned int *)m_expandedKey[0][3]);
}
